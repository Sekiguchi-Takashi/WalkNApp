...
## 現在地
- v2.3: 画面の見切れ修正＋通知ストップ＋ケイデンス帯別の歩幅学習
  - 全画面に statusBarsPadding / navigationBarsPadding を適用。上部パネルがステータスバーに、下部ボタンがナビゲーションバーに重ならないようにした
  - 下部の4ボタンを NavButton に統一し contentPadding を詰めて「記録」の文字折り返しを解消
  - 通知に「ストップ」アクションを追加。歩きながらアプリを開かずに終了できる
  - 歩幅をケイデンス3帯（ゆっくり<1.6歩/秒 / ふつう1.6-2.0 / 速い>2.0）で個別に学習
    - GPS良好な区間だけ帯ごとに距離と歩数を蓄積し、150歩以上たまった帯のみ更新
    - 更新は旧値7:新値3の加重平均。速く歩くほど歩幅が伸びる個人差を反映できる
  - Room version 6（loadout の stride_m を stride_low / stride_mid / stride_high に置換）
